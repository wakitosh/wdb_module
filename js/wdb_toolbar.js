...
(function ($, Drupal, drupalSettings, once) {
  ...
  const ICONS = {
    ...
    // Direction icons (provided SVG content, scaled via viewBox)
    dir_ltr: '<g>\
	<path d="M413.8,435.1l-53.9-53.9c-5.3-5.3-13.8-5.3-19.1,0c-5.3,5.3-5.3,13.8,0,19.1l30.9,30.9H121.2c-7.4,0-13.5,6-13.5,13.5\
		s6,13.5,13.5,13.5h250.4L340.8,489c-5.3,5.3-5.3,13.8,0,19.1c2.6,2.6,6.1,3.9,9.5,3.9s6.9-1.3,9.5-3.9l53.9-53.9\
		C419,448.9,419,440.4,413.8,435.1L413.8,435.1z"/>\
	<path d="M417.7,0H188.6C129.2,0,80.8,48.3,80.8,107.8s48.3,107.8,107.8,107.8h27v121.3c0,7.4,6,13.5,13.5,13.5\
		c7.4,0,13.5-6,13.5-13.5V27h80.9v309.9c0,7.4,6,13.5,13.5,13.5c7.4,0,13.5-6,13.5-13.5V27h67.4c7.4,0,13.5-6,13.5-13.5\
		S425.1,0,417.7,0z M215.6,188.7h-27c-44.6,0-80.9-36.3-80.9-80.9S144.1,27,188.6,27h27V188.7z"/>\
</g>',
    dir_rtl: '<g>\
	<path d="M417.7,0H188.6C129.2,0,80.8,48.3,80.8,107.8s48.3,107.8,107.8,107.8h26.9v121.3c0,7.4,6,13.5,13.5,13.5\
		c7.4,0,13.5-6,13.5-13.5V26.9h80.8v309.9c0,7.4,6,13.5,13.5,13.5s13.5-6,13.5-13.5V26.9h67.4c7.4,0,13.5-6,13.5-13.5\
		S425.1,0,417.7,0z M215.6,188.6h-26.9c-44.6,0-80.8-36.3-80.8-80.8s36.3-80.8,80.8-80.8h26.9V188.6z"/>\
	<path d="M390.7,431.2H140.3l30.9-30.9c5.3-5.3,5.3-13.8,0-19.1s-13.8-5.3-19.1,0l-53.9,53.9c-5.3,5.3-5.3,13.8,0,19.1l53.9,53.9\
		c2.6,2.6,6.1,3.9,9.5,3.9c3.4,0,6.9-1.3,9.5-3.9c5.3-5.3,5.3-13.8,0-19.1l-30.9-30.9h250.4c7.4,0,13.5-6,13.5-13.5\
		S398.2,431.2,390.7,431.2L390.7,431.2z"/>\
</g>',
  };
  ...
  function buildAndShowThumbnailPager(pagerContainer, pageList, currentPage, pageNavigation) {
    ...
    pageList.forEach(function (pageData) {
      ...
      // Load thumbnails in CORS mode so cross-origin IIIF servers that send a
      // Cross-Origin-Resource-Policy header do not block them (NotSameSite).
      ...
      once('wdb-toolbar-init', '#wdb-panel-toolbar', context).forEach(function (panelToolbar) {
        ...
        if (pagerContainer && osdSettings.pageList && osdSettings.pageList.length > 0) {
          ...
          const makeNavButton = (type, targetIndex) => {
            const title = !isRTL
              ? (type === 'prev' ? Drupal.t('Previous Page') : Drupal.t('Next Page'))
              : (type === 'prev' ? Drupal.t('Next Page') : Drupal.t('Previous Page'));
        ...
        // --- Direction Icon (LTR / RTL) ---
        try {
          const dir = (osdSettings.pageNavigation || 'left-to-right');
          // Avoid duplicating if it already exists.
          if (!panelToolbar.querySelector('.wdb-dir-badge')) {
            const badge = document.createElement('span');
            badge.className = 'wdb-dir-badge order-dir';
            badge.setAttribute('aria-hidden', 'true');
            badge.title = (dir === 'right-to-left')
              ? Drupal.t('Reading direction: Right-to-Left')
              : Drupal.t('Reading direction: Left-to-Right');
            const svgIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
            // Keep SVG sized like other toolbar icons; scale via viewBox.
            svgIcon.setAttribute('viewBox', '0 0 512 512');
            svgIcon.innerHTML = (dir === 'right-to-left') ? ICONS.dir_rtl : ICONS.dir_ltr;
            badge.appendChild(svgIcon);
            if (panelToolbar.firstChild) {
              panelToolbar.insertBefore(badge, panelToolbar.firstChild);
            } else {
              panelToolbar.appendChild(badge);
            }

            // Insert a flexible gap right after the badge once, to push other buttons to the right.
            if (!panelToolbar.querySelector('.wdb-toolbar-gap')) {
              const gap = document.createElement('span');
              gap.className = 'wdb-toolbar-gap';
              if (badge.nextSibling) {
                panelToolbar.insertBefore(gap, badge.nextSibling);
              } else {
                panelToolbar.appendChild(gap);
              }
            }
          }
        } catch (e) {
