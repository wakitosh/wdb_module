    ...
    attach: function (context, settings) {
      once('wdb-toolbar-init', '#wdb-panel-toolbar', context).forEach(function (panelToolbar) {
        ...
        if (osdSettings.isEditable) {
          if (urls.view) {
            panelToolbar.appendChild(createToolbarButton(Drupal.t('View Mode'), urls.view, false, ICONS.view));
          }
        }
        else {
          if (urls.edit) {
            panelToolbar.appendChild(createToolbarButton(Drupal.t('Edit Annotations'), urls.edit, false, ICONS.edit));
          }
        }
        ...
        if (urls.manifest_v3) {
          panelToolbar.appendChild(createToolbarButton(Drupal.t('View IIIF Manifest'), urls.manifest_v3, true, ICONS.iiif));
        }

        // Event listener for the dropdown.
        exportButton.addEventListener('click', (e) => {
          // If the clicked element is one of the download links inside,
          // let the browser's default action (navigation) proceed.
          if (e.target.classList.contains('wdb-dropdown-item')) {
            // Close the dropdown just in case.
            dropdown.classList.remove('is-visible');
            return;
          }

          // Otherwise, assume the outer button was clicked,
          // prevent the default link action, and toggle the dropdown's visibility.
          e.preventDefault();
